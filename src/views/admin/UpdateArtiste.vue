<template>
  <main class="mx-6 mb-6 sm:mx-32 md:mx-44 md:mt-6 lg:mx-64 lg:grid lg:grid-cols-2 lg:gap-40">
    ...
    <form @submit.prevent="updateArtiste">
      ...
      <div class="w-full bg-[#f6c139] p-3 text-center font-barlow text-xl font-bold uppercase text-gray-100">Sélectionner une image</div>
      <input
        type="file"
        placeholder="Nom de l'artiste"
      ...
      <div class="flex flex-row justify-between gap-5">
        <button type="submit" class="w-full bg-[#f6c139] p-3 font-barlow text-xl font-bold uppercase text-gray-100">Modifier</button>

        <button class="w-full bg-[#f6c139] p-3 font-barlow text-xl font-bold uppercase text-gray-100">
...
export default {
  ...
  methods: {
    ...
    async updateArtiste() {
      if (this.imgModifiee) {
        const storage = getStorage();
        let docRef = ref(storage, "artiste/" + this.imageActuelle);
        deleteObject(docRef);
